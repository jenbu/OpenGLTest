...
namespace test
{
    ...
    : m_Colors{ 1.0f, 1.0f, 1.0f, 1.0f },
      m_Proj(glm::ortho(0.0f, 300.0f, 0.0f, 540.0f, -1.0f, 1.0f)), m_View(glm::translate(glm::mat4(1.0f), glm::vec3(0, 0, 0))),
      ...
      m_RotationA(0.5f), m_RotationB(0.0f)
    {
        ...
        RectangleObject rectA("RectA", 300, 300), rectB("RectB", 400, 400);
    ...
    void TestRectangle2D::OnImGuiRender()
    {
        ...
        for(auto& rect: m_Rectangles)
        {
            ...
            glm::mat4 model = glm::translate(glm::mat4(1.0f), *translation)*glm::rotate(glm::mat4(1.0f), m_RotationA, glm::vec3(0.0f, 0.0f, 1.0f));
            ...
            ImGui::SliderFloat3(rect.GetName().c_str(), &translation->x, 0.0f, 960.0f);
        }

        if(ImGui::Button("Add Square"))
            m_Rectangles.push_back(RectangleObject(std::to_string(m_Rectangles.size()), 500, 500));

        ImGui::SliderFloat("Rotation", &m_RotationA, 0.0f, 3.14f);
